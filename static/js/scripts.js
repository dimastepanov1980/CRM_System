            ...
            fetch(`/specialist/${uuid}/detail/`)
                ...
                .then(data => {
                    ...
                    specialistDetailContainer.innerHTML = `
                        ...
                        <p><strong>specialist ID:</strong> ${data.specialist_id}</p>
                        <input type="hidden" id="specialist-id" value="${data.specialist_id}">
                    `;

                    // Запрос событий специалиста и обновление календаря
                    fetch(`/specialist/${uuid}/events/`)
                    .then(response => {
                        if (!response.ok) {
                            throw new Error('Network response was not ok ' + response.statusText);
                        }
                    ...
                    .then(events => {
                        updateCalendar(events, data.specialist_id);
                    })
                    .catch(error => {
